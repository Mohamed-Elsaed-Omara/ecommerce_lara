    ...
    <div class="card shadow mb-4">
        ...
        <form action="{{ url('admin/slides/'.$slide->id) }}" method="POST" enctype="multipart/form-data">
            @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select class="custom-select" name="product_id">
                        ...
                        @foreach ($products as $product)
                            <option value="{{$product->id}}" {{ $product->id == $slide->product_id ? 'selected' : '' }} > {{$product->product_name}}</option>
                        @endforeach
                    </select>
                        @error('product_id')
                ...
                <div class="form-group">
                    <label>Content</label>
                    <textarea name="content" class="form-control content" rows="10">{{ $slide->content }}</textarea>
                    @error('content')
                    <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
